...
happy <- function( datafile, allelesfile, generations=200, phase="unknown",
  ...
  mother = vector( mode="character", length=length(h$mother))
  w = which(h$mother>0)
  mother[w] = h$subjects[h$mother[w]]
  w = which(h$mother<=0)
  mother[w] = NA
  h$mother = mother

  father = vector( mode="character", length=length(h$father))
  w = which(h$father>0)
  father[w] = h$subjects[h$father[w]]
  w = which(h$father<=0)
  father[w] = NA
  h$father = father
...
mergedpositionmatrix <- function( h, position, prepmerge, model='additive', verbose=FALSE, design=TRUE ) {
  ...
  if ( ! is.null(ind) ) {
    ...
    if ( design ) { # return the design matrix
      ...
      retval <- hdesign( h, marker, mergematrix=mergematrix, model=model )
    }
...
epistasis <- function( h, markers1, markers2=NULL, merge1=NULL, merge2=NULL, model='additive', verbose=FALSE, family='gaussian' ) {
  ...
  if ( is.null( markers2 ) ) { # all pairwise interactions 
    ...
    for ( m1 in 2:nmarkers ) {
      ...
      print(ma1)
      for( m2 in 1:(m1-1) ) {
        results[r,] <- epistasispair( h, ma1, markers1[m2], merge1=merge1, merge2=merge2, model=model, verbose=verbose, d1=d[[m1]], d2=d[[m2]], main1=main[[m1]], main2=main[[m2]] )
        r <- r+1
      }
    }
  }
...
save.genome <- function ( gdir, sdir, prefix, chrs=NULL,
  ...
  if ( ! require(multicore)) mc.cores = 1
...
load.genome <- function (sdir,
                         use.X = TRUE,
                         chrs   = the.chromosomes(use.X=use.X),
                         n.chr=NA,
                         models=c("additive", "full", "genotype"))  # CHANGED
{
  ...
  old.strains <- NULL
  if ( is.integer(n.chr) )
    chrs = paste("chr", 1:n.chr, sep="")
  
  for (model in models) {
    if ( file.exists( paste(sdir, model, sep = "/") )) {
    pkgs <- paste(sdir, model, chrs, sep = "/")   # CHANGED
